...
public class thotInit extends Thread {
   static int thread_no = 0;

   public static void main (String args[]) {
      new thotThread(thread_no++);
      // new thotThread(thread_no++);
   }
}

class thotThread extends Thread {
   protected int no;

   public thotThread(int no) {
      this.no = no;
      System.out.println("thotThread " + no);
      this.start();
   }

   public void run() {
        System.out.println("thotThread" + no + "started");
	while (true) {
	   try {
	      Thread.sleep(5000);
	   }
	   ...
	   //thotlib.Interface.TtaSetCursorWatch(1, 1);
	   //thotlib.Interface.TtaSetStatus(1, 1, "Busy ...", "Thot");
	   //try {
	   //   Thread.sleep(500);
	   //}
	   //catch (InterruptedException e) {}
	   //catch (Exception e) {}
	   //catch (Throwable e) {}
	   //thotlib.Interface.TtaResetCursor(1, 1);
	   //thotlib.Interface.TtaSetStatus(1, 1, "Done", "Thot");
	}
        // System.out.println("thotThread" + no + "stopped");
   }
}
