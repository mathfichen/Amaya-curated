/*                                                W3C Reference Library libwww Response Class
                                    THE RESPONSE CLASS
...
/*
   ...
   The response object is created as a placeholder for the response received by the remote
   server. All filters can then use the information passed in the response and act
   appropriately. The response objetc is deleted automatically when the corresponding
   request object is deleted. We try and do some fancy tricks in order to do lazy parsing
   and reusing parsed values so that we can optimize the code.
   
   This module is implemented by HTResponse.c, and it is a part of the W3C Reference
   Library.
...
CREATING AND DELETING RESPONSE OBJECTS
   ...
   The response obejct gets created when we start receiving a response from a remote
   server.
...
HTTP CONNECTION CONTROL DIRECTIVES
   ...
   behavior of this connection. The respose object contains the connection information
   that we have received in the response.
...
HTTP EXTENSIONS (PEP)
  ...
  Protocol
   ...
   This association list is a list of the extension directives that were received as part
   of the response.
...
HTTP CACHE CONTROL DIRECTIVES
   ...
   This association list is a list of the cache control directives that have been received
   as part of the response. We also keep track of whether the response is cachable or not.
...
/*
  ...
  Is the response Cachable?
   ...
   The various cache-controlheaders and directives decides whether an object is cachable
   or not. Check these methods before starting caching!
...
/*
  ...
  Has the Response been Cached?
   ...
   When we gc the response object we need to know whether the header lists have been
   inherited by other objects (the anchor object, for example) and hence shouldn't be
   deleted. We can tell the response object this by using the following method call
...
/*
  ...
  Content Type (Media type)
   ...
   We store the content-type of the response along with the charset, level and all other
   paramaters that may follow the content-type itself.
...
/*
   ...
   The Response object stores all content type parameters (charset, level, etc.) in an
   Association list so here you will always be able to find them. We also have a few
   methods for the special cases: charsetand levelas they are often needed.
 ...
 */
extern HTEncoding HTResponse_transfer (HTResponse * response);
extern BOOL HTResponse_setTransfer (HTResponse * response, HTEncoding cte);
/*

  Original Response Headers
  
   The MIME parser may add the original response headers as (name,value) pairs. The
   information may be picked up by the persistent cache manager.
...
/*
   ...
   ___________________________________
                         ...
                         @(#) $Id: HTResponse.h,v 1.1.1.1 1996/10/15 13:08:41 cvs Exp $
