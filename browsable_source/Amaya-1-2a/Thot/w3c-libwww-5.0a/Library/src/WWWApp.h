/*                                         W3C Reference Library libwww Application Interface
...
/*
   ...
   In addition top the basic W3C Reference Library WWWLib interface you mayinclude the
   other interfaces depending on the needs of your application. However, it is not
   ...
   if the functionality you are after is covered by them. This interface contains many
   application specific features including a set of default BEFORE and AFTER filters.
...
/*
  ...
  Event Manager
   ...
   The core part of libwww only provides the hooks for the event manager. There is no
   event loop internal to the core part. Instead the application must provide the event
   loop in order to use either pseudo threads or real threads. If the application only
   uses blocking sockets without threads then it is not required to register any event
   loop at all. We provide a default implementation of an event loop which you can either
   take or get some ideas from.
...
/*
  ...
  Load, Upload, and Search URLs
   ...
   Even though you may use the API for the HTRequest object directly in order to issue a
   request, you will probably find that in real life it is easier to use a higher level
   abstraction API. This API is provided by the HTAccess module where you will find all
   kind of functions for down loading a URL etc.
...
/*
  ...
  BEFORE and AFTER Filters
   ...
   _Before_ a request has been issued and _after_ it has terminated the application often
   has to do some action as a result of the request (and of the result of the request).
   The Client Profile Interface Library provides a set of standard BEFORE and AFTER
   filters to handle _caching_, _redirection_, _authentication_, _logging_ etc.
...
/*
   ...
   ___________________________________
                            ...
                            @(#) $Id: WWWApp.h,v 1.1.1.1 1996/10/15 13:08:39 cvs Exp $
