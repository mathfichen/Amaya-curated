...
prologue(Method* meth)
{
	...
	slot_slot_const(0, 0, (jword)meth, HAVE_prologue, Tnull);
}
...
exception_prologue(void)
{
	...
	slot_slot_slot(0, 0, 0, HAVE_exception_prologue, Tnull);
}
...
void
_fixup_function_call(uintp pc, uintp stk)
{
	_slot_const_const(0, stk, pc, fixupFunctionCall, Tnull);
}
...
void
softcall_new(SlotInfo* dst, Class* classobj)
{
	prepare_function_call();
...
void
softcall_anewarray(SlotInfo* dst, SlotInfo* size, Class* type)
{
	slot_nowriteback(size);
	prepare_function_call();
	pusharg_int(size, 1);
...
void
softcall_multianewarray(SlotInfo* dst, int size, SlotInfo* stktop, Class* classobj)
{
	int i;
...
void
softcall_checkcast(SlotInfo* obj, Class* class)
{
	/* Must keep 'obj' */
	prepare_function_call();
	pusharg_ref(obj, 1);
...
void
softcall_instanceof(SlotInfo* dst, SlotInfo* obj, Class* class)
{
	slot_nowriteback(obj);
	prepare_function_call();
	pusharg_ref(obj, 1);
