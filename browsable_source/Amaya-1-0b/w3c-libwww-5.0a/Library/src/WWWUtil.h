/*                                               W3C Reference Library libwww Basic Utilities
                   DECLARATION OF W3C REFERENCE LIBRARY BASIC UTILITIES
...
/*
   ...
   The Basic Utility modules are mostly container modules, dynamic strings and other stuff
   that you can't live without when building applications. The modules are used by the
   core part of the W3C Reference Library but can also be used by the application. See
   also the main Library include file called WWWLib.h which contains all converters and
   protocol modules known to the Library. You can include this one if the application is
   to use all the functionality of the Library.
...
/*
  ...
  Association Lists
   ...
   Name-Valuepairs in an easy way.
...
/*
  ...
  Dymamic Memory Management
   ...
   required that an application can handle memory exhaustion gracefully.
...
/*
  ...
  UU encode and decode
   ...
   character (or more aptly, 3 binary bytes to 4 ASCII characters). However, RFC 1113 does
   not use the same mapping to printable characters as uuencode.
...
/*
   ...
   ___________________________________
                           ...
                           @(#) $Id: WWWUtil.h,v 1.1.1.1 1996/10/15 13:08:38 cvs Exp $
