/*                                                    W3C Reference Library libwww Link Class
                                      THE LINK CLASS
...
/*
   ...
   objects that we have created. Anchor objects are bound together using Link objects.
   Each anchor can be the source or destination of zero, one, or more links from and to
   other anchors.
   
   This module is implemented by HTLink.c, and it is a part of the W3C Reference Library.
...
HANDLE LINK BETWEEN ANCHORS
   ...
   As mentioned, link objects are the ones that bind anchor objects together in a Web like
   structure
...
/*
  ...
  Find a Link
   ...
   Find the anchor object between a destination and a source ancher. Return link object if
   any, else NULL
...
/*
  ...
  Link Types and Semantic Links
   ...
   Each link has a sematic representation associated with it. This means that the
   application can distinguish between pages based on the semantics of the link. This is
   very similar to the LINKtag in HTML pages which indicates the meaning if this pages to
   other pages.
...
/*
  ...
  Link Method
   ...
   The link method is the HTTP method we have performed between the two links. For
   example, it can be a POST operation, or a PUT operation where the operation on the
   first anchor created a new anchor.
...
/*
  ...
  Link Result
   ...
   the result of the operation is stored as part of the link information. This means that
   we can keep track of which operations we have performed on a link and hence the
   application can ask the user whether he or she wants to do a re-post, for example.
...
/*
   ...
   ___________________________________
                             ...
                             @(#) $Id: HTLink.h,v 1.1.1.1 1996/10/15 13:08:41 cvs Exp $
