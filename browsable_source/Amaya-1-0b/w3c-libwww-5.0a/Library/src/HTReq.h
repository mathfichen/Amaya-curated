/*                                                 W3C Reference Library libwww Request Class
                                    THE REQUEST CLASS
...
/*
   ...
   Request object. The request object is application independent in that both servers and
   clients use the same Request class. Examples of requests passed to the Library are a
   client application issuing a _GET_ request on a HTTP URL, or a server issuing a load on
   a local file URL. The only difference is that the client gets the input from a user
   whereas the server gets the input via the network.
   
   A request object is created with a default set of parameters which are applicable for
   many URL requests but the class defines a huge set of methods that an be used to
   customize a request for a particular purpose. Example of things that you can define is
   natural language, media types, what RFC 822 headers to use, whether the request should
   be refreshed from cache etc. Scroll down and see the set of parameters you can tune.
   
   A request object is registered in the library by issuing an operation on a URL - for
   example _PUT_, _POST_, or _DELETE_. You can find many higher level "request issuing
   functions" in the Access module - the methods defined by the Request class itself are
   very low level but can of course be used directly if needed.
   
   Whereas the lifetime of the URL (in form of an anchor) often is very long (for example
   as long as the application is running), the lifetime of a request is limited to the
   time it takes to service the request. The core does not automatically delete any
   request object created by the application - it is for the application to do. In many
   cases a request object can be deleted when any of the termination callback functions
   are called but the application may keep request objects around longer than that
   
   The Library can accept an unlimited number of simultaneous requests passed by the
   application. One of the main functions of the Library core is to handle any number of
   ongoing requests in an intelligent manner by limiting the number of active request to
   the fit the available resources as defined by the application. This is described in
   more detail in the HTNet module.
...
ISSUING A REQUEST
   ...
   This is the "_basic request issue method_" provided by the Request class. This is a
   very low level API as the caller must have set up the request object before passing it
   to the Library. You can find many higher level issuing functions in the HTAccess
   module. If you like, you can of course use this directly!
...
CREATION AND DELETION METHODS
   ...
   be deleted as soon as it has terminated, for example in one of the request termination
...
/*
  ...
  Clear a Request Object
   ...
   another request. It should be use with care as application specific information is
   _not_ re-initialized. Returns YES if OK, else NO.
...
/*
   ...
   Creates a new HTRequest object as a duplicate of the src request. The difference to the
   HTRequest_dup function is that we don't copy the error_stack and other information that
   the application keeps in its copy of the request object. Otherwise it will be freed
   multiple times. Returns YES if OK, else NO
...
DATE AND TIME STAMP WHEN REQUEST WAS ISSUED
   ...
   The start time when the request was issued may be of value to the cache validation
   mechanism as described by the HTTP/1.1 specification. The value is automatically set
   when creating the request headers and sending off the request. The time is a local
   time.
...
BINDING TO A USER PROFILE
   ...
   Each request is associated with a User profile which contains information about the
   local host name, email address of the user, news server etc. A request object is
   created with a default "generic user" but can be assigned a specific user at any time.
...
BINDING TO A RESPONSE OBJECT
   ...
   If a request is actually going on the net and we are getting a response back then we
   also create a HTResponse object and bind it to the request object. Once we know what to
   do with the response, we may transfer the information to the anchor object.
...
ERROR OBJECT
   ...
   some easy mapping functions to the HTError object, so that you can add an error
...
/*
   ...
   These are the cover functions that go directly to the Error Object
...
SET MAX FORWARDS FOR TRACE METHODS
   ...
   The TRACEmethod is used to invoke a remote, application-layer loop-back of the request
   ...
   in the request. A TRACErequest _MUST NOT_ include an entity.
...
/*
  ...
  Request Headers
   ...
   default value is all request headers if present exceptFromand Pragma.
...
LOCAL MIME HEADER PARSERS
   ...
   MIMEParsers get their own type which is optimized for static and regex parser strings.
...
ACCEPT HEADERS
   ...
   allHTTP requests and a local set of accept headers that are used in specific requests
   ...
   global set or replaces the global set of accept headers. Non of the headers haveto be
   set. If the global set is sufficient for all requests then this us perfectly fine. If
   the parameter "override" is set then only local accept headers are used, else bothlocal
   and global headers are used.
   
  Content Types
  
   The locallist of specific conversions which the format manager can do in order to
   fulfill the request. It typically points to a list set up on initialisation time for
   example by HTInit(). There is also a globallist of conversions which contains a generic
...
HTTP CACHE VALIDATION AND CACHE CONTROL
   ...
   request. The mempory buffer is considered to be equivalent to a history buffer. That
   is, it doesn't not follow the same expiration mechanism that is characteristic for a
   persistent file cache.
   
   You can also set the cache to run in disconnected mode - see the Cache manager for more
...
/*
  ...
  HTTP Cache Control Directives
   ...
   The cache control directives are all part of the cache control header and control the
   behavior of any intermediate cache between the user agent and the origin server. This
   association list is a list of the connection control directives that are to be sent as
...
/*
  ...
  Partial Requests and Range Retrievals
   ...
   Libwww can issue range requests in case we have already obtained a part of the entity
   body. Since all HTTP entities are represented in HTTP messages as sequences of bytes,
   the concept of a byte range is meaningful for any HTTP entity. (However, not all
   clients and servers need to support byte-range operations.) Byte range specifications
   in HTTP apply to the sequence of bytes in the entity-body (not necessarily the same as
   the message-body). A byte range operation may specify a single range of bytes, or a set
...
HTTP CONNECTION CONTROL REQUEST DIRECTIVES
   ...
   The connection control directives are all part of the connection header and control the
   behavior of this connection. This association list is a list of the connection control
   directives that are to be sent as part of the Connectionheader.
...
HTTP ACCESS AUTHENTICATION CREDENTIALS
   ...
   When a access denied response is returned to the Library, for example from a remote
   HTTP server, this code is passed back to the application. The application can then
   decide whether a new request should be established or not. These two methods return the
   authentication information required to issue a new request, that is the new anchor and
   any list of keywords associated with this anchor.
...
HTTP EXTENSIONS (PEP)
   ...
   HTTP can be extended in several ways but traditionally it has been by using new
   headers. Here we present a new idea which provides a framework for describing
   extensions and their scope. This is only an idea an may be modified later! The
   implementation of the extensions can be found in the PEP module
   
  Protocol
  
   This association list is a list of the extension directives that are to be sent as part
   of the request.
...
BEFORE AND AFTER FILTERS
   ...
   The request object may have it's own before and after filters. These may override or
   suplement the global set in HTNet. The request object itself handles the list element,
   that is this should not be freed bu the caller.
   
  BEFORE Filters
  
   The BEFORE filters are called just after the request has been passed to the Library but
   before any request is issued over the network. A BEFORE can infact stop a request
   completely from being processed.
   
    Add a local BEFORE Filter
    
   You can add a local _BEFORE_ filter for a single request so that the both the local and
   global _BEFORE_ filters are called or you can replace the global filters with a local
   set. Note that the local set can be NULL. This can be used to effectively disable all
   _BEFORE_ filters without unregistering the global ones.
...
/*
   ...
   You can delete a local BEFORE filter explicitly by passing the filter itself or you can
   delete all filters which are registered for a certain status code.
...
/*
  ...
  AFTER Filters
   ...
   You can add a local AFTER filter for a single request so that the both the local and
   global AFTER filters are called or you can replace the global filters with a local set.
   Note that the local set can be NULL. This can be used to effectively disable all AFTER
   filters without unregistering the global ones.
   
   AFTER filters can be registered to handle a certain set of return values from the
   protocol modules, for example explicitly to handle redirection, authentication, etc.
   You can find all the available codes in the HTNet object description.
...
/*
   ...
   You can delete a local AFTER filter explicitly by passing the filter itself or you can
   delete all filters which are registered for a certain status code.
...
SENDING DATA TO THE NETWORK
   ...
   Multiple Request objects can be connected in order to create a PostWeb for sending data
   from one location (source) to another (destination). Request objects are bound together
   by connecting the output stream of the source with the input stream of the destination
   requst. The connection can be done directly so that the output from the source is
   exactly what is sent to the destination or there can be a conversion between the two
   streams so that we can do conversions on the fly while copying data. This is in fact
   the way we use for building a proxy server.
   
   The Library supports two ways of posting a data object to a remote destination: Input
   comes from a socket descriptor or from memory. In the case where you want to copya URL,
   for example from local file system orfrom a remote HTTP server then you must use the
   PostWeb design. This model operates by using at least two request objects which gets
   linked to eachother as part of the PostWeb model. However, if you are posting from
   memory, we only use onerequest object to perform the operation. In order to do this,
...
/*
   ...
   The Entity Anchor is either the anchor directly associated with the Request object or
   the post anchor associated with the object. The purpose of the entity anchor is if we
   are to send data to a remote server then we get the metainformation using the entity
   anchor.
...
/*
  ...
  Input Stream
   ...
   The input stream is to be used to put data tothe network. Normally each protocol sets
   the input stream in order to generate the protocol headers while making a request.
...
/*
  ...
  Source of a Request
   ...
   A request may have a source in which is another request object that as output stream
   has the input stream of this request object.
...
STREAMS FROM NETWORK TO APPLICATION
  ...
  Default Output Stream
   ...
   The output stream is to be used to put data down to as they come in _from_ the network
   and back to the application. The default value is NULLwhich means that the stream goes
   to the user (display).
...
/*
  ...
  Has Output Stream been Connected to Channel?
   ...
   Has output stream been connected to the channel? If not then we must free it explicitly
   when deleting the request object
...
/*
  ...
  Debug Stream
   ...
   All object bodies sent from the server with status codes different from 200 OKwill be
...
/*
   ...
   from the library. The default value if WWW_HTMLas a character based only has one
   WWW_PRESENT.
...
BINDING TO AN ANCHOR OBJECT
   ...
   being serviced. If the anchor that we have requested is infact a child anchor then we
   always load the parent anchor and then after the load jump to the location. A child
   anchor is a an anchor which points to a subpart of the document (has a "#" in the URL).
...
SHOULD WE ISSUE A FULL HTTP REQUEST-URI?
   ...
   In early versions of HTTP, the request sent to the remote server varies whether we use
   a proxy or go directly to the origin server. The default value is OFFbut we use a full
   request if we are talking to a proxy server.
...
PROXY URL
   ...
   In case we are using a proxy for this requst then we can register it together with the
   request object. That way we can find the proxy and look for authentication information,
   for example in the Authentication filter. The string is freed by the Request object on
   deletion.
...
INTERNAL REQUEST OBJECTS
   ...
   The library may under certain circumstances create its own Request objects. These are
   all handled internal and does not show up on the application side at all.
...
/*
   ...
   ___________________________________
                             ...
                             @(#) $Id: HTReq.h,v 1.1.1.1 1996/10/15 13:08:39 cvs Exp $
